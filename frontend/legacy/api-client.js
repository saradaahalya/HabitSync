...
const APIClient = {
  ...
  async logoutUser() {
    try {
      console.log("APIClient: Sending logout request to backend...");
      const response = await this.request('/auth/logout', {
        method: 'POST'
      });
      console.log('APIClient: Backend logout response:', response);
      return response;
    } catch (err) {
      console.error('APIClient: Backend logout failed:', err);
      // Don't throw - allow logout to proceed even if backend fails
